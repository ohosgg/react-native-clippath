...
namespace rnoh {
    ...
    void ClipPathViewComponentInstance::onPropsChanged(SharedConcreteProps const &props) {
        ...
        if (auto p = std::dynamic_pointer_cast<const facebook::react::RNCClipPathViewProps>(props)) {
            ...
            //             shadowOffsetX: " << p->shadowOffsetX; LOG(INFO) << "[clx]
            //             <ClipPathViewComponentInstance::setProps> shadowOffsetY: " << p->shadowOffsetY; LOG(INFO) <<
            //             "[clx] <ClipPathViewComponentInstance::setProps> shadowOffsetIsPercent: "
            //                       << p->shadowOffsetIsPercent;
            //             LOG(INFO) << "[clx] <ClipPathViewComponentInstance::setProps> bgColor: " << p->bgColor;
